...
use eponym_world::{BodyKind, BoxCollider, ContactWorld, HeldInput, Input, TriggeredInput};
...
fn airborne_release_needs_no_support_and_lands_without_tunnelling() {
    use eponym_world::{ContactEffect, MovableBoard};
    let mut world = ContactWorld::new(vec![BoxCollider {
        min: [-10., -0.01, -10.],
        max: [10., 0., 10.],
    }]);
...
fn old_contact_input_grammar_is_explicitly_rejected() {
    assert_eq!(
        ...
        Err(eponym_world::ContactError::UnsupportedVersion(1))
    );
}
