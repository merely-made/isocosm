...
fn setup() -> (Session, Vec<ItemId>, [i32; 3]) {
    let mut game =
        eponym_world::GameState::new(World::generate(4242, WorldConfig::default()).unwrap());
    let at = game
        .items()
        .all()
...
fn stale_or_missing_anatomy_rejects_body_change_atomically() {
    ...
    let mut missing_game = eponym_world::GameState::new(session.game().world().clone());
...
fn save_restore_continues_body_change_and_lethal_fall_cannot_resurrect() {
    ...
    assert!(matches!(
        ...
        Err(SessionError::Game(GameError::Body(
            eponym_world::BodyError::Dead(SUBJECT)
        )))
    ));
