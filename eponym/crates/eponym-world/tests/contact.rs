...
fn restore_rejects_bad_initial_configuration_and_frame_order() {
    let invalid_profile = eponym_world::ContactBodyProfile {
        half_extents: [0.0, 0.4, 0.4],
        ..eponym_world::ContactBodyProfile::crawler()
    };
    ...
    assert!(matches!(
        ...
        Err(eponym_world::ContactError::UnsupportedVersion(9))
    ));
    ...
    assert!(matches!(
        ...
        Err(eponym_world::ContactError::InvalidFrameTick { .. })
    ));
}
...
fn configuration_locks_after_first_tick_and_rejects_out_of_range_input() {
    ...
    assert!(matches!(
        ...
        Err(eponym_world::ContactError::ConfigurationLocked)
    ));
    assert!(matches!(
...
fn grip_impairment_prevents_reanchor_and_attack_cooldown_applies_on_hit() {
    ...
    assert_eq!(
        ...
        Some(eponym_world::Impairment::Grip)
    );
    assert!(!world.body(climber).unwrap().anchored);
    world
        .step(&[(
