...
//! a Eponym session to that crate's product-neutral inputs, and nothing else.
//!
...
//! # Declared limits
//!
//! - **Anatomies have no voxel data.** Eponym bodies address volumes by
//!   [`isometer::core::VolumeRef::from_tag`] and store no voxels, so each intact
...
//!   the world's are voxels; nothing in `eponym-world` relates them, so
//!   [`Appearance::scale`] owns the conversion and defaults to [`BODY_SCALE`].
//! - **No per-part material expression is projected**: Eponym has no
