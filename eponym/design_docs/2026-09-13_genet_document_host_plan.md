...
| Piece | Where | State |
| --- | --- | --- |
| Connected session driven by keyboard and mouse | `eponym-client` `timed_action` bin | Text HUD only; movement, strikes, injury, rest, dressing pickup, save/load |
| Subject sheet and equipment panels | `eponym-client::body_sheet` | *Retired 2026-09-15; see "Retiring the body sheet" below.* Was a private `EquipmentSession` with fixed subject 1 |
...
A `eponym-client` module implementing `cambium_rootstock::TextureProducer`
...
A new `eponym-client` bin running `cambium_genet_winit_host::run` with a
...
A `genet-probe` scenario under `eponym/testing/session/` that moves the
...
**Scope.** A `eponym_world::glyphs` reading (world crate, because the
...
wing-glyphs dependency; P4 adds it to `eponym-world` only.
...
**What it was.** `eponym-client::body_sheet`, 2,388 lines across nine files,
...
| Equipment persistence (`equipment_store`) | `eponym_client::equipment_store`, moved unchanged |
...
They are owned by `eponym-world/tests/equipment.rs` and stay there. Its
...
- **The authored three-lives comparison mode.** It compared three fixture
  ...
  and resources. `eponym-world/tests/three_lives.rs` still proves the
...
- **`EquipmentSession`'s load guard.** `load_bytes` refused a save whose
  ...
  claim is owned by `eponym-world/tests/equipment.rs`.
...
Stage exact owned paths under `eponym/` only. Mesocosm's benchmark and
...
- **isometer-core (ruled 2026-09-14).** Mark chose the physical split:
  ...
  lands, eponym-world and eponym-client take body and ground from
  ...
  *Landed 2026-09-15:* eponym-world takes isometer-core directly
  (63ef2c5); eponym-client followed each rename (63ef2c5, 48d70d7) and
...
- **2026-09-14, P1 landed.** `eponym_client::producer` (root plus
...
- **2026-09-14, P2 landed.** The `session` bin (working name) runs the
  ...
  The seed-7 fixture moved to `eponym_client::session_fixture`; the
  ...
  field is empty for the picked part; that projection is eponym-world's.
...
- **2026-09-14, P3b landed.** The session bin is the second wing-scenario
  ...
  `eponym/testing/session/acceptance.scenario` moves, strikes, treats,
...
- **2026-09-14, P4 landed.** `eponym_world::glyphs`: `GlyphRules` for
...
- **2026-09-14, P5 landed** (the two follow-ups P3b and P4 left open). The
  seed-7 fixture has one home: `eponym_world::fixtures::session`, public and
  unconditional like `three_lives`, built from this crate's own dependencies
  only. `eponym-world/src/glyphs/fixture.rs` and
  `eponym-client/src/session_fixture.rs` are both gone; `glyphs::tests` reads
