...
- **2026-09-09:** local `nisus` and `modulus` provide shared mechanics, while
  `eponym-world/Cargo.toml` pins Conatus to `d82afa17`. Adoption must respect
...
- **2026-09-09, J1a implementation:** `eponym-world::Session` owns one
...
- **2026-09-09, J1a verification:**
  `cargo test -p eponym-world --lib --test session_boundaries --locked --offline
...
- **2026-09-09, save growth baseline:** the new
  `crates/eponym-world/examples/save_growth.rs` checks restored equality and
...
- 2026-09-09: core lifecycle landed in source with transactional charge/release
  and injury batches. A full `eponym-world` run passed 91 tests. After the final
...
Run the native surface with `./scripts/wing.ps1 eponym run -p eponym-client
...
`crates/eponym-world/tests/fixtures/timed-action-v1-game-v3.save` is the verbatim
...
- 2026-09-13: full `eponym-world` suite passes **104 tests** on the aligned
  ...
  `eponym-client --bin timed_action` passes **3 handler tests**. The runnable
...
Reproduce from the umbrella with `./scripts/wing.ps1 eponym` followed by:

```text
test -p eponym-world --offline --locked --target-dir target-contact -j 2
test -p eponym-client --bin timed_action --offline --locked --target-dir target-contact -j 2
build -p eponym-client --bin timed_action --offline --locked --target-dir target-contact -j 2
...
`crates/eponym-world/tests/fixtures/timed-action-v1-game-v5.save` has SHA-256
