...
`crates/eponym-client` (lib plus `src/bin/room.rs`), the repo's first
game code.
...
Picture: `ROOM_TRACE=1 cargo run -p eponym-client --bin room` opens a
...
`ROOM_R1=1 cargo run -p eponym-client --features r1-proof --bin room`
...
`cargo run -p eponym-client --features v1-proof --bin v1_residency`
...
`cargo run -p eponym-client --features v1b-proof --bin v1b_residency`
...
headed run `cargo run -p eponym-client --features d1-proof --bin d1_depth`
...
judgment is open.** The scene is `crates/eponym-social` (lib,
`src/bin/refusal.rs`, `tests/refusal.rs`); the §1 identity facts are
`crates/eponym-identity`. 33 tests green across the workspace,
...
settles. `cargo run -p eponym-social --bin refusal` prints the premises
...
**What landed.** `settlement.rs` in `eponym-social`: dwellings,
...
**Open: the headed judgment**, as S1's. `cargo run -p eponym-social
...
**What landed.** `crates/eponym-sortie`, the one crate allowed to read
...
`cargo run -p eponym-sortie --bin sortie` prints the judgment surface
...
`eponym-world` projects Mesocosm's current verb-neutral `Grown` topology and
...
Verification on 2026-08-21: all fourteen `eponym-world` tests pass and
...
eighteen `eponym-world` tests pass; focused and workspace clippy pass with
...
**Landed 2026-08-26.** `eponym-social::epistemic` meets that bounded
...
`eponym-identity` and `BodyDocument`/`PartId` from Mesocosm for anatomical
...
| --- | --- |
| Continuing subject and body revision | `eponym-identity::{SubjectId, BodyRevisionId}`; the query must reject mismatched subjects and stale revision references. |
...
| Item identity and possession | `eponym-world::Items` owns carriage and now dressing attachments addressed by subject/part through `GameState`. Harness/line attachments and supplied functions in the three-lives query remain caller-supplied fixture inputs. |
...
player-control selector. `eponym-world` supplies read-only presentation
rows from body/query facts; `eponym-client` owns selection, navigation,
...
cast, initial facts, selected rule profile, and replay inputs. `eponym-world`
owns admitted material/body/action outcomes; `eponym-social` owns knowledge,
...
`cargo run -p eponym-client --bin crossing` opens the authored dry cut, loose
...
larger world. `eponym-world::ContactWorld` owns profile limits, action
...
Verification: `cargo test -p eponym-world --tests -p paredros-room --test
...
- **2026-09-24, two S3 receipts fail before and after the family rename:**
  `cargo test --workspace --no-fail-fast` reports 230 passed, 2 failed, 3
  ignored, both on the pre-rename tree and on the renamed one. The failures
  are `an_injury_persists_as_a_body_revision_fact` ("the played body was
  never wounded", `crates/eponym-sortie/tests/sortie.rs:131`) and
  `a_tag_in_occurs_mid_action_under_the_pact` ("the pact never fired",
  `sortie.rs:65`), both reproducible. Likely cause, not yet investigated:
  `eponym-sortie` reads `mesocosm-core` by path
  (`crates/eponym-sortie/Cargo.toml:19`) and its terrain-fall wound and pact
  tag-in happen on grown Mesocosm terrain, which mesocosm-core's ecology
  commits of 2026-09-16 (42552a4, 42bb3c3, accb92d) likely shifted, so the
  scripted fall and the pact trigger no longer occur. The sortie crate itself
  is unchanged since 2026-09-13. Open for a separate lane; the rename did not
  touch it.
...
- **2026-09-05 (encounter design):** `eponym-world/src/transitions.rs`
...
- **2026-08-08 (S3):** `Wound` lives in `eponym-sortie` for now. The
...
- **2026-08-08 (S2):** the settlement lives in `eponym-social` as a
...
- **2026-08-08 (S1):** the two-owners rule is a dependency fact, and that
  ...
  side. `crates/eponym-identity` is therefore a foundation crate that
...
- **2026-08-08 (S1):** `eponym-social` takes a **dev**-dependency on
  ...
  only `serde` and `eponym-identity`.
...
- **2026-09-08: equipment persistence automated gate passed.** Using
  ...
  `cargo test -p eponym-world -p paredros-room --lib --bin body_sheet --bin crossing --test equipment --test anatomy_admission --test f1_state --test f2_simulation --test three_lives --test crossing --test contact --test contact_actions --locked --offline -j 2 --target-dir target-contact`.
...
- **2026-09-06: durable anatomy admission prerequisite.** Terra implemented
  ...
  Reproduce with:
  `cargo test -p eponym-world -p paredros-room --lib --bin body_sheet --bin crossing --test anatomy_admission --test f1_state --test f2_simulation --test three_lives --test crossing --test contact --test contact_actions --locked --offline -j 2 --target-dir target-contact`.
...
- **2026-09-06: native Body/Actions inspector.** Luna supplied the read-only
  ...
  Reproduce with:
  `cargo test -p eponym-world -p paredros-room --lib --bin body_sheet --bin crossing --test three_lives --test crossing --test contact --test contact_actions --locked --offline -j 2 --target-dir target-contact`.
...
- **2026-09-06: three-lives/action-query local slice.** Terra implemented
  ...
  `cargo build -p eponym-world -p paredros-room --example three_lives --locked --offline -j 2 --target-dir target-contact`.
...
- **2026-09-06: crossing release fix implemented locally.** Added independent
  ...
  `cargo test -p eponym-world --test contact --test contact_actions -p paredros-room --test crossing --bin crossing --locked --offline -j 2 --target-dir target-contact`.
...
- **2026-08-26 (later):** D1 landed — brick raymarch and the renderling
  ...
  in passing, not D1's doing: two `eponym-sortie` sim-content tests
...
- **2026-08-26:** F3 became active and F3a landed in `eponym-social`: an
...
- **2026-08-08:** S3's sim half landed; headed real-time action open;
  R4 armed. `crates/eponym-sortie` joins the workspace as the one
...
- **2026-08-08:** S2 landed with its headed judgment open. `settlement.rs`
  and the settling scene join `eponym-social`: homes offered with daily
...
- **2026-08-08:** S1 landed with its headed judgment open.
  `crates/eponym-social` is the willingness owner (deeds, standing,
  confidence, refusal, standing agreements, premises) and
  `crates/eponym-identity` is the foundation both owners share. Three
