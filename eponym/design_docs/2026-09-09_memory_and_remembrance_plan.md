...
- `eponym-social::DeedLog` is append-only and owns social deeds. A deed has
...
- `eponym-social::EpistemicLog` is append-only and observer-scoped. It
...
- `eponym-world::GameState` owns the subject-addressed transition log and
...
- Ordinary memory is owned by the individual epistemic subsystem in
  `eponym-social`; durable history remains owned by the deed/event journals
...
`eponym-world` remains the authority for accepted world transitions, actor
...
should not decide what an individual cares about. `eponym-social` owns
...
- `eponym-world::glyphs`: the reading tracks the live canon revision from
