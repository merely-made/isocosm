...
- `eponym-world` already separates requests from accepted results:
  `WorldIntent`/`WorldEvent` in `crates/eponym-world/src/world.rs` and
...
`eponym-world`. Preserve `WorldIntent`/`WorldEvent` ownership. Do not alter
...
- **2026-09-09:** Plan drafted from the Paredros founding/execution records,
  local `eponym-world` seams, Mesocosm's digestible graft-affinity precedent,
