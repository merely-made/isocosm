...
impl BodyLayer {
    ...
    pub fn glyph_anchors(
        ...
        anchors(body.subject, revision, &mesh, posed(body, &mesh), selected)
    }
}
...
pub(crate) fn anchors(
    ...
    if selected.is_some_and(|s| s.subject != subject || s.revision != revision) {
        return Err("glyph attachment selection has expired".into());
    }
    let mut placements: Vec<_> = mesh
        .placements
        .iter()
    ...
    for placement in placements {
        ...
        let quad = &part_mesh.quads[index];
