//! Direct board manipulation: what a gesture does, the context menu, undo.
//!
//! The editor's own verbs. A tile arrives already resolved — [`gestures`] owns
//! the pane-local point and the pick that turns it into a `TileCoord` — so
//! nothing below knows where a pointer is, which is why B3 could change how a
//! click finds its tile without touching one rule about what a click means.
//!
//! Pane-local coordinates still reach two things here: the context menu, which
//! is anchored where the press landed and drawn inside that same container,
//! and the camera pan.
//!
//! Split out of `state.rs` on 2026-07-24; the gestures moved in from the
//! desktop host on 2026-09-02 when it became the shared one. B3 sent the
//! pointer-facing half to [`gestures`] on 2026-09-15, when resolving a point
//! stopped being one call to the inverse projection: what is left here is what
//! a gesture *ends in* — the editor verbs, the turn order, the undo stack —
//! and none of it knows where a pointer is.
