...
//!
//! One authored rule per `(effect, acquiring act)` pair says how an owned
//! effect returns. `resolve` emits a presentation-neutral [`MarkRequest`]: it
...
/// The one base effect the default pack carries three acquiring-act rules on.
...
pub const MASS_CAP_MG: u64 = 4000;
...
/// The journey-rule axis: the acquiring act, three poles, each already
/// selected by the runtime adapter's accepted-event filter.
...
/// The forms the three poles return as. A host maps these to its own
/// orientation and placement; this enum names no renderer type.
...
pub struct PackRule {
    ...
    pub color: [u8; 4],
    pub cost: CostShape,
    /// Free text naming the acquisition evidence this rule answers to. The
    /// explanation template reads it; nothing re-derives it from event names.
    pub citation: String,
}
...
impl EffectPackTable {
    ...
    /// The three ruled rules on one glyph (2026-09-15): carving-earned returns
    /// as a per-use surface inscription, feeding-earned as an emission that
    /// lives only while feeding continues, movement-earned as a free trail.
    pub fn default_pack() -> Self {
        Self {
            rules: vec![
                PackRule {
                    effect: DEFAULT_EFFECT.into(),
                    acquired_by: Acquiring::Carved,
                    ...
                    citation: "the Carved grant's evidence \
                        mesocosm.trial/{baseline}/{post}/history/{sequence} and its canon revision"
                        .into(),
                },
                PackRule {
                    effect: DEFAULT_EFFECT.into(),
                    acquired_by: Acquiring::Fed,
                    ...
                    citation: "the Fed grant's evidence \
                        mesocosm.trial/{baseline}/{post}/history/{sequence} and its canon revision"
                        .into(),
                },
                PackRule {
                    effect: DEFAULT_EFFECT.into(),
                    acquired_by: Acquiring::Moved,
                    form: MarkForm::PathTrail,
                    stroke: Glyph::Slashes,
                    color: [115, 242, 204, 255],
                    cost: CostShape::Free,
                    citation: "the Moved grant's evidence \
                        mesocosm.trial/{baseline}/{post}/history/{sequence} and its canon revision"
                        .into(),
                },
            ],
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for rule in &self.rules {
            ...
            if !seen.insert((rule.effect.as_str(), rule.acquired_by)) {
                return Err(format!(
                    "duplicate pack rule for {} acquired by {:?}",
                    rule.effect, rule.acquired_by
                ));
            }
        }
        Ok(())
    ...
    /// carries several acquiring-act rules with different cost shapes, so the
    ...
    pub fn rule(&self, effect: &str, acquired_by: Acquiring) -> Option<&PackRule> {
        self.rules
            .iter()
            .find(|r| r.effect == effect && r.acquired_by == acquired_by)
    }
    ...
    pub fn resolve(
        ...
        let rule = self.rule(effect, acquired_by).ok_or(Refusal::NoRule {
            effect: effect.to_owned(),
        })?;
        ...
        let explanation = format!(
            "{:?} acquired by {:?} returns as {:?}; cited to {}. {} {} sets scale {}‰ over {} ticks.",
            rule.stroke,
            acquired_by,
            rule.form,
            rule.citation,
    ...
    pub fn resolve_for_glyph(
        ...
        self.resolve(effect, owned, acquired_by, at, to, amount)
    }
}
