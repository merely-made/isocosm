...
pub fn view(state: &Bench) -> Child {
    ...
    Box::new(el("div", (
        ...
        custom_leaf::<Bench, ()>(LEAF_KEY, 640, 400)
            ...
            .attr("role", "img").attr("aria-label", "Specimen population"),
        el("div", vec![
            button("Turn left", |s| s.turn(-0.2617994)),
            button("Turn right", |s| s.turn(0.2617994)),
            button("Reset pose", |s| { let mut m=s.model.borrow_mut(); m.yaw=0.; m.changed(); }),
            button("Natural", |s| s.tint=0),
            button("Warm", |s| s.tint=1),
            button("Cool", |s| s.tint=2),
        ]).attr("class", "toolbar"),
        el("p", text(state.published_error.clone().unwrap_or_else(|| "Ready for population inspection.".into()))).attr("role", "status"),
