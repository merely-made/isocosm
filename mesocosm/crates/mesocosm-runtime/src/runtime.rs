...
impl Runtime {
    ...
    /// Builds a driver from a world that already has a past (D4): deep time's
    /// handover, not a fresh one. `epoch_seen` starts at the world's own
    /// epoch and `history` carries the past that got it there, so the first
    /// tick reckons only a boundary that closes during it, not the gap
    /// between an empty log and a world already several epochs in.
    fn from_world_with_history(
        world: World,
        history: History,
        seed: u64,
        organisms: u32,
...
#[cfg(test)]
mod deep_time_fixture;
#[cfg(test)]
use deep_time_fixture::deep_time_world;
