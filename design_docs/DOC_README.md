...
- [Orthographic voxel presentation](../mesocosm/design_docs/2026-09-11_orthographic_voxel_presentation_plan.md):
  ...
  A bounded disposable idle trial preserves source generation and projects accepted movement and feeding records through the same scene. General CSS 3D, large-DOM scaling, planar retention, the appearance crate merger and broader world-intent trials remain separate work.
...
| [2026-09-15_board_on_isometer_plan.md](2026-09-15_board_on_isometer_plan.md) | **All six isometer additions and B1 to B2 landed 2026-09-15.** The board through the wing's shared scene, `isometer`, instead of one DOM element per tile and token, keeping the locked 2:1 lens, the tileset-as-stylesheet contract and every tile and token selectable. The scene additions are in: a terrain material palette, a terrain hit on the pick, an integer render scale, tokens as live bodies from their voxel recipes, a per-material body palette and a dimetric camera preset. The map now reads as terrain through one tile-kind colour table that also generates the stylesheet, and `ISOMETRY_SCENE_BOARD=1` draws the board as one scene leaf whose picks agree with the DOM board on every probe pixel over flat ground. Mark ruled the cliff height on B2's finding: a tile is five voxels across and an elevation step two voxels tall, landing within a fifth of a pixel of the DOM step. Selection, overlays and the switch (B3 to B5) remain, and the improvements found in isometer are recorded for its owner. |
...
| [2026-08-23_runtime_profile_plan.md](2026-08-23_runtime_profile_plan.md) | **Active behind the protocol and host prerequisites.** Product-owned `isometry-runtime` conducts accepted zero-step Conatus changes into an optional `conatus::resident` body plane (formerly Quint) and configurable fixed-isometric GPU tenant, then composes its same-device texture through Netrender. Desktop wiring waits for protocol H2 and the Genet host migration; shared conductor, source, frame, lease, and tenant contracts wait for a second game. |
