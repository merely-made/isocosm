...
| [2026-08-09_cleromancy_generator_selection.md](2026-08-09_cleromancy_generator_selection.md) | **Decision record; carries no status line of its own, but states acceptance criteria and a stop rule, with publishing the paired source revisions named as a separate release/commit step.** Cleromancy chooses one already-loaded Isometry generator before the ordinary host-side preview call, entered as `>choose <seed> <domain> <prompt>`. Cleromancy owns qualification, derived selection and the sealed receipt; Isometry owns the command, the loaded generator declarations, preview execution, the host entropy tape and the campaign commit. The receipt is host-local: not a `GameEvent`, not in a campaign checkpoint, not sent to joined players. Added to this index 2026-09-18; W1 found it active and unlisted, an index defect. |
