...
| [2026-09-15_board_on_isometer_plan.md](2026-09-15_board_on_isometer_plan.md) | **Approved 2026-09-15, lanes starting.** The board through the wing's shared scene, `isometer`, instead of one DOM element per tile and token, keeping the locked 2:1 lens, the tileset-as-stylesheet contract and every tile and token selectable. Five isometer additions first (material palette, terrain hit, integer render scale, tokens as live bodies from their voxel recipes, a dimetric camera preset), then the board behind a flag until parity. Records the improvements found in isometer for its owner. |
