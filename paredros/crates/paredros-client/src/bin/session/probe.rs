...
pub(super) fn default_out_dir() -> PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .map(|paredros| paredros.join("testing").join("session"))
...
const HELP: &str = "\
paredros-client session — one played Paredros session as a Cambium document.
...
under paredros/testing/session/; acceptance receipts are kept in
paredros/testing/session/receipts/<date>/.";
